"""
Area Export Manager - Gestión de exportación e importación de areas

Permite exportar areas completos a JSON con todas sus relaciones y componentes,
e importar areas desde archivos JSON.
"""
...
class AreaExportManager:
    """Gestor de exportación e importación de areas"""

    def __init__(self, db_manager):
        self.db = db_manager

    def export_area(self, area_id: int, file_path: str = None) -> Optional[str]:
        """
        Exporta un proyecto completo a JSON

        Args:
            area_id: ID del proyecto a exportar
        ...
        try:
            ...
            if not area:
                logger.error(f"Proyecto {area_id} no encontrado")
                return None
            ...
            if not file_path:
                safe_name = "".join(c for c in area['name'] if c.isalnum() or c in (' ', '-', '_')).strip()
    ...
    def import_area(self, file_path: str, import_mode: str = 'new') -> Optional[int]:
        """
        ...
        try:
            ...
            # Validar estructura
            if 'Area' not in import_data:
                logger.error("Archivo JSON inválido: falta 'Area'")
                return None

            area_data = import_data['Area']
            ...
            existing_names = [p['name'] for p in all_areas]

            if area_name in existing_names:
                # Agregar sufijo numérico
                counter = 1
                while True:
                    new_name = f"{area_name} ({counter})"
                    if new_name not in existing_names:
                        area_name = new_name
                        break
                    counter += 1

                logger.info(f"Nombre duplicado, renombrando a: {area_name}")

            area_id = self.db.add_area(
                name=area_name,
                description=area_data.get('description', ''),
                color=area_data.get('color', '#9b59b6'),
            ...
            logger.info(f"Proyecto '{area_data['name']}' creado con ID: {area_id}")
            ...
            if 'components' in import_data:
                for component in import_data['components']:
                    success = self.db.add_area_component(
                        area_id=area_id,
                        component_type=component['component_type'],
                        content=component.get('content', ''),
                        order_index=component.get('order_index', 0)
    ...
    def export_all_areas(self, output_dir: str = "exports") -> List[str]:
        """
        Exporta todos los areas activos a archivos JSON individuales
        ...
        try:
            ...
            # Obtener todos los areas activos
            areas = self.db.get_all_areas(active_only=True)

            exported_files = []
            for area in areas:
                safe_name = "".join(c for c in area['name'] if c.isalnum() or c in (' ', '-', '_')).strip()
                safe_name = safe_name.replace(' ', '_')
                file_path = Path(output_dir) / f"{safe_name}_{area['id']}.json"

                result = self.export_area(area['id'], str(file_path))
                if result:
                    exported_files.append(result)

            logger.info(f"Exportados {len(exported_files)} areas a {output_dir}")
            return exported_files

        except Exception as e:
            logger.error(f"Error exportando todos los areas: {e}", exc_info=True)
            return []

    def get_export_summary(self, area_id: int) -> Dict[str, Any]:
        """
