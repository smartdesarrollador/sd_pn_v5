"""
Area Editor Dialog - Diálogo para editar información de un área
"""
...
class AreaEditorDialog(QDialog):
    """Diálogo para editar información de un área"""

    area_updated = pyqtSignal(int)  # area_id

    def __init__(self, area_data: dict, db_manager, parent=None):
        """
        Args:
            area_data: Diccionario con datos del área
        ...
        self.db = db_manager
        self.selected_color = area_data.get('color', '#9b59b6')

        self.init_ui()

    def init_ui(self):
        """Inicializa la interfaz"""
        self.setWindowTitle("✏️ Editar Área")
        ...
        # Nombre
        name_label = QLabel("Nombre del Área:")
        name_label.setStyleSheet("font-weight: bold; color: #ffffff;")
        layout.addWidget(name_label)

        self.name_input = QLineEdit()
        self.name_input.setText(self.area_data.get('name', ''))
        self.name_input.setPlaceholderText("Nombre del área...")
        ...
        self.description_input.setPlainText(self.area_data.get('description', ''))
        self.description_input.setPlaceholderText("Descripción del área...")
        ...
        self.icon_input.setText(self.area_data.get('icon', '🏢'))
        ...
        delete_btn = QPushButton("🗑️ Eliminar Área")
        delete_btn.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
        delete_btn.setStyleSheet(self._get_button_style("#e74c3c"))
        delete_btn.clicked.connect(self.on_delete_area)
        ...
        save_btn.setStyleSheet(self._get_button_style("#9b59b6"))
    ...
    def on_save(self):
        ...
        if not name:
            QMessageBox.warning(self, "Error", "El nombre del área es requerido")
            return

        try:
            # Actualizar área
            success = self.db.update_area(
                self.area_data['id'],
                name=name,
            ...
            if success:
                logger.info(f"Area {self.area_data['id']} updated")
                self.area_updated.emit(self.area_data['id'])
                self.accept()
            else:
                QMessageBox.warning(self, "Error", "No se pudo actualizar el área")

        except Exception as e:
            logger.error(f"Error updating area: {e}")
            QMessageBox.critical(self, "Error", f"Error al actualizar:\n{str(e)}")

    def on_delete_area(self):
        """Al hacer clic en eliminar área"""
        reply = QMessageBox.question(
            self,
            "Confirmar eliminación",
            f"¿Eliminar el área '{self.area_data['name']}'?\n\n"
            "Esto eliminará todas las relaciones y componentes del área.\n"
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                success = self.db.delete_area(self.area_data['id'])
                if success:
                    logger.info(f"Area {self.area_data['id']} deleted")
                    QMessageBox.information(self, "Éxito", "Área eliminada")
                    self.accept()
                else:
                    QMessageBox.warning(self, "Error", "No se pudo eliminar el área")
