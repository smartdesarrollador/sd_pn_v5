"""
Area Tag Selector Widget - Selector multi-tag con autocompletado
...
from src.views.widgets.area_tag_chip import AreaTagChip


class AreaTagSelector(QWidget):
    """
    ...
    def __init__(self, tag_manager: AreaElementTagManager, parent=None):
        ...
        self.selected_tags: List[AreaElementTag] = []
    ...
    def _on_tag_selected(self, item: QListWidgetItem):
        ...
        if tag is None:
            # Crear nuevo tag
            query = self.search_input.text().strip()
            tag = self.tag_manager.create_tag(name=query, color="#9b59b6")
    ...
    def _add_chip(self, tag: AreaElementTag):
        """Agrega un chip de tag a la vista"""
        chip = AreaTagChip(tag, removable=True)
    ...
    def _on_chip_removed(self, tag_id: int):
        ...
        for i in range(self.chips_layout.count()):
            ...
            if isinstance(widget, AreaTagChip) and widget.tag.id == tag_id:
                widget.deleteLater()
                break
